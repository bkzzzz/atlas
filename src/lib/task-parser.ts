...
export function getTaskParserModel() {
  return process.env.OPENAI_TASK_PARSER_MODEL?.trim() || DEFAULT_MODEL;
}
...
export function safeMessageFor(category: ParserErrorCategory) {
  return {
    ...
    timeout: "The parser timed out. Please try again.",
    malformed_structured_output: "The parser returned an invalid structured task. Please try again.",
    not_configured: "The task parser is not configured on this server.",
    unknown_upstream_error: "Could not parse the art request. Please try again.",
  }[category];
}
...
export function classifyParserError(error: unknown): TaskParserError {
  ...
  const diagnostic: ParserDiagnostic = apiError
    ? {
  ...
  if (
    ...
    return new TaskParserError("timeout", diagnostic);
  }
  if (diagnostic.code === "insufficient_quota" || diagnostic.type === "insufficient_quota") {
    return new TaskParserError("insufficient_quota", diagnostic);
  }
  if (diagnostic.code === "rate_limit_exceeded" || diagnostic.type === "rate_limit_exceeded") {
    return new TaskParserError("rate_limit_exceeded", diagnostic);
  }
  if (
    diagnostic.code === "model_not_found" ||
    diagnostic.type === "model_not_found" ||
    diagnostic.status === 404
  ) {
    return new TaskParserError("model_not_found", diagnostic);
  }
...
export type ParseStaticImageTaskResult = {
...
function usageFrom(response: {
  ...
  return {
    ...
    estimatedCostUsd: Number(
      ((inputTokens * INPUT_COST_PER_MILLION + outputTokens * OUTPUT_COST_PER_MILLION) / 1_000_000).toFixed(8),
    ),
  };
}
...
export async function parseStaticImageTask(
  ...
  try {
    const response = await createClient().responses.create(
      {
        model: getTaskParserModel(),
        store: false,
        max_output_tokens: 500,
        instructions:
          "Extract one static 2D game-art image brief. Return only the supplied JSON schema. Preserve stated visual details and use concise assumptions only when needed. Do not choose a provider, operation, mode, animation, edit, batch, or output count. Describe one still image even when the request uses motion-related wording.",
        input: request,
        text: {
          format: {
            type: "json_schema",
            name: "atlas_static_image_task",
            strict: true,
            schema: staticImageTaskSchema,
          },
        },
      },
